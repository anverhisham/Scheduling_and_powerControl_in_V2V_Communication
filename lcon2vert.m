...
% -Got this code from http://www.mathworks.com/matlabcentral/fileexchange/30892-representing-polyhedral-convex-hulls-by-vertices-or-inequalities/content/lcon2vert.m
%   Note: Each row of output corresponds to corresponding vertex!
...
% 
   ...
   if strictinpoly(b,TOL)  
       ...
       c=zeros(dim,1);
            ...
            %- Comment_Anver: A weakly interior point 'c' in polytope is found out now.
           ...
           if ~strictinpoly(s,TOL) %Added 02/17/2012 to handle initializers too close to polytope surface
                ...
                [bmin,idx]=min(bb);
                 ...
                 if bmin>=-TOL
%                  if all(bb<=-TOL)       %% Anver: Possible Bug fixed here??...   
...
function c=Initializer2(TOL,A,b,c)
 %norm(  (I-A*pinv(A))*(s-b) )  subj. to s>=0 
    ...
    while ii<=2*maxIter %HARDCODE
     ...
     s=s-Aaugt*(Aaug*(s-b))./C;   %% Comment_Anver: Couldn't understand division by C ??
     s(s<0)=0;
