...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Find max-volume ellipsoid under linear constraints
%   Got this code from http://www.caam.rice.edu/~zhang/mve/ 
%
% -NOTE:
%   1. This code is returning -ve center ('x') for 100 dimensional box constraints!     SO NOT WORKING! 
%   2. Replace all '%fprintf' with '%fprintf' to make it verbose 
%   3. To see code changes by Anver, search "Anver"
%
% -TODO:
%   1. Understand this algorithm by reading the above problem.
%
% -Reference:
%  1. Yin Zhang and Liyan Gao. On Numerical Solution of the Maximum Volume Ellipsoid Problem. SIAM Journal on Optimization, Vol.14, No.1, pp. 53-76, 2003.
%       http://epubs.siam.org/doi/pdf/10.1137/S1052623401397230
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


%%
function [x,E,exitStatus] = findMaxVolumeEllipsoidInPolytope(A,b,x0)
...
[m, n] = size(A); 
t0 = tic;
x = zeros(n,1); E = eye(n); exitStatus = 0;

%% - Check for non-emptiness of the Polytope
[xFeasible,~,exitFlag] = linprog([],A,b,[],[],[],[],[],optimoptions('linprog','Display','none'));
if(exitFlag~=1 || max(A*xFeasible-b)>0)
    exitStatus = 1; 
    return;
end
% %% -Finding an 'x0' seems to improve performance (Added by Anver Hisham)
% if(gf.isempty('x0'))
%     [x0,~,exitFlag] = linprog([],A,b,[],[],[],[],[],optimoptions('linprog','Display','none'));
%     if(exitFlag~=1)
%         x0 = [];
%     end
% end
...
if(gf.isempty('x0'))
  ...
  if msg(1) ~= 's'  %% -If no volume
      %%disp(msg);  %%fprintf 
      x = linprog([],A,b,[],[],[],[],[],optimoptions('linprog','Display','none'));  %% Return any feasoble point
      E = zeros(numel(x));
      return; 
  end
...
try                         
    E = chol(E2); E = E';
%     E = E2;                     %% Temporarily added by Anver for testing purposes
%     E = (E+E')/2;             %% -Added by Anver, since we need E to be positive semidefinite, hence symmetric(This is possibly a bug in his code!!, Need to report him later)
catch                   %% If E is not positive definite
    exitStatus = 1;
    return;
end
%fprintf('  [m, n] = [%i, %i]\n', m, n);
